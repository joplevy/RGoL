...
from tensorflow.keras.layers import Dense, Dropout, Activation, Flatten, Conv2D, MaxPooling2D, Input, LSTM
...
start = []
delta = []
with open("./resources/train.csv", "r") as fd:
...
delta = np.array(delta).astype(np.int32)
stop = np.array(stop).astype(np.int32)
start = np.array(start).astype(np.int32)
data_input = zip(delta, stop)
print(data_input.shape)
print (stop[0])
print (start.shape)
# print (delta.shape)

# stop = stop.reshape(-1, 20, 20, 1)
# start = start.reshape(-1, 20, 20)
# print (stop.shape)
...
# model = Dense([
# input0 = InputLayer(shape=(400, 1), dtype='int32', name='input0')
# input1 = InputLayer(shape=(1, 1), dtype='int32', name='input1')
# i0layer = LSTM(400, ret÷urn_sequences=False)(input0)
# i1layer = LSTM(8 , return_sequences=False)(input1)
# # x = concatenate(
# 	[
# 		i0layer,
# 		i1layer,
...
# model = tf.keras.Model(inputs=[input0, input1], outputs=predictions)
# ])
# x = Dense(400, activation='relu')(x)
# predictions = Dense(400, activation='relu')(x)

model = tf.keras.Sequential(
	[
		layers.Dense(400, activation='relu', input_shape=(400, 1)),
...
# Trains for 5 epochs
# model.fit(zip(stop, delta), start, epochs=5)

# model.compile(optimizer='adam',
# 				loss='sparse_categorical_crossentropy',
# 				metrics=['accuracy'])

# model.fit(stop, start, epochs=5)
